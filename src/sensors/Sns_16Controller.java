 ...
 * @author sandeepk
 */
public class Sns_16Controller implements Initializable {

    private static Sns_16Controller instance;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        try {
            Platform.runLater(() -> {
                lbl_sensor_tag.setText("" + Variable.sensor_details[Variable.sensor_details_sequence_16][Constant.SENSOR_TAG]);
                ...
                if (Variable.sensor_details[Variable.sensor_details_sequence_16][Constant.SENSOR_TYPE_2D].equalsIgnoreCase("true")) {
                    lbl_sensor_type.setText("2D");
                    lbl_sensor_address_vertical.setText("" + Variable.sensor_details[Variable.sensor_details_sequence_16][Constant.SENSOR_ADDRESS_V]);

                } else {
                    lbl_sensor_type.setText("3D");
                    lbl_sensor_address_vertical.setText("" + Variable.sensor_details[Variable.sensor_details_sequence_16][Constant.SENSOR_ADDRESS_V]);
                    lbl_sensor_address_horizontal.setText("" + Variable.sensor_details[Variable.sensor_details_sequence_16][Constant.SENSOR_ADDRESS_H]);
                }

                lbl_sensor_ip_address.setText("" + Variable.sensor_details[Variable.sensor_details_sequence_16][Constant.SENSOR_IP_ADDRESS]);
                lbl_sensor_port_number.setText("" + Variable.sensor_details[Variable.sensor_details_sequence_16][Constant.SENSOR_PORT_NUMBER]);
                try {
    ...
    public Sns_16Controller() {
        instance = this;
    }

    public Sns_16Controller get_Controller() {
        return instance;
    }
