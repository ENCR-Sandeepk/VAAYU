...
public class Client_9 {
    ...
    public static void send_command(String cmd) {
        try {
            if (Constant.DEBUG) {
                System.out.println("CMD 9 : " + cmd);
            }
            cmd = cmd + "\r\n";
            Variable.got_reply_9 = false;
            Variable.reply_9 = "";
            temp_rly = "";
    ...
    public static void start_server(String host, int port) {
        new Thread() {
            ...
            public void run() {
                try {
                    if (Constant.DEBUG) {
                        System.out.println("HOST 9 : " + host);
                        System.out.println("PORT 9 : " + port);
                    }
                    ...
                    while (((len > -1))) {
                        try {
                            ...
                            if (temp_rly.contains("\r")) {
                                Variable.reply_9 = temp_rly;
                                temp_rly = temp_rly.substring(temp_rly.indexOf("\r") + 1);
                                if (Constant.DEBUG) {
                                    System.out.println("RLY 9  = " + Variable.reply_9);
                                }
                                Variable.got_reply_9 = true;
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
