...
public class Client_10 {
    ...
    public static void send_command(String cmd) {
        try {
            if (Constant.DEBUG) {
                System.out.println("CMD 10 : " + cmd);
            }
            cmd = cmd + "\r\n";
            Variable.got_reply_10 = false;
            Variable.reply_10 = "";
            temp_rly = "";
    ...
    public static void start_server(String host, int port) {
        new Thread() {
            ...
            public void run() {
                try {
                    if (Constant.DEBUG) {
                        System.out.println("HOST 10 : " + host);
                        System.out.println("PORT 10 : " + port);
                    }
                    ...
                    while (((len > -1))) {
                        try {
                            ...
                            if (temp_rly.contains("\r")) {
                                Variable.reply_10 = temp_rly;
                                temp_rly = temp_rly.substring(temp_rly.indexOf("\r") + 1);
                                if (Constant.DEBUG) {
                                    System.out.println("RLY 10  = " + Variable.reply_10);
                                }
                                Variable.got_reply_10 = true;
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
