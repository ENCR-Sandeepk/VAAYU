...
public class Client_5 {
    ...
    public static void send_command(String cmd) {
        try {
            if (Constant.DEBUG) {
                System.out.println("CMD 5 : " + cmd);
            }
            cmd = cmd + "\r\n";
            Variable.got_reply_5 = false;
            Variable.reply_5 = "";
            temp_rly = "";
    ...
    public static void start_server(String host, int port) {
        new Thread() {
            ...
            public void run() {
                try {
                    if (Constant.DEBUG) {
                        System.out.println("HOST 5 : " + host);
                        System.out.println("PORT 5 : " + port);
                    }
                    ...
                    while (((len > -1))) {
                        try {
                            ...
                            if (temp_rly.contains("\r")) {
                                Variable.reply_5 = temp_rly;
                                temp_rly = temp_rly.substring(temp_rly.indexOf("\r") + 1);
                                if (Constant.DEBUG) {
                                    System.out.println("RLY 5  = " + Variable.reply_5);
                                }
                                Variable.got_reply_5 = true;
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
