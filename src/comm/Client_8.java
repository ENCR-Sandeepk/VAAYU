...
public class Client_8 {
    ...
    public static void send_command(String cmd) {
        try {
            if (Constant.DEBUG) {
                System.out.println("CMD 8 : " + cmd);
            }
            cmd = cmd + "\r\n";
            Variable.got_reply_8 = false;
            Variable.reply_8 = "";
            temp_rly = "";
    ...
    public static void start_server(String host, int port) {
        new Thread() {
            ...
            public void run() {
                try {
                    if (Constant.DEBUG) {
                        System.out.println("HOST 8 : " + host);
                        System.out.println("PORT 8 : " + port);
                    }
                    ...
                    while (((len > -1))) {
                        try {
                            ...
                            if (temp_rly.contains("\r")) {
                                Variable.reply_8 = temp_rly;
                                temp_rly = temp_rly.substring(temp_rly.indexOf("\r") + 1);
                                if (Constant.DEBUG) {
                                    System.out.println("RLY 8  = " + Variable.reply_8);
                                }
                                Variable.got_reply_8 = true;
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
