...
public class Client_7 {
    ...
    public static void send_command(String cmd) {
        try {
            if (Constant.DEBUG) {
                System.out.println("CMD 7 : " + cmd);
            }
            cmd = cmd + "\r\n";
            Variable.got_reply_7 = false;
            Variable.reply_7 = "";
            temp_rly = "";
    ...
    public static void start_server(String host, int port) {
        new Thread() {
            ...
            public void run() {
                try {
                    if (Constant.DEBUG) {
                        System.out.println("HOST 7 : " + host);
                        System.out.println("PORT 7 : " + port);
                    }
                    ...
                    while (((len > -1))) {
                        try {
                            ...
                            if (temp_rly.contains("\r")) {
                                Variable.reply_7 = temp_rly;
                                temp_rly = temp_rly.substring(temp_rly.indexOf("\r") + 1);
                                if (Constant.DEBUG) {
                                    System.out.println("RLY 7  = " + Variable.reply_7);
                                }
                                Variable.got_reply_7 = true;
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
