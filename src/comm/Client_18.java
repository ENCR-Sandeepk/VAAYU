...
public class Client_18 {
    ...
    public static void send_command(String cmd) {
        try {
            if (Constant.DEBUG) {
                System.out.println("CMD 18 : " + cmd);
            }
            cmd = cmd + "\r\n";
            Variable.got_reply_18 = false;
            Variable.reply_18 = "";
            temp_rly = "";
    ...
    public static void start_server(String host, int port) {
        new Thread() {
            ...
            public void run() {
                try {
                    if (Constant.DEBUG) {
                        System.out.println("HOST 18 : " + host);
                        System.out.println("PORT 18 : " + port);
                    }
                    ...
                    while (((len > -1))) {
                        try {
                            ...
                            if (temp_rly.contains("\r")) {
                                Variable.reply_18 = temp_rly;
                                temp_rly = temp_rly.substring(temp_rly.indexOf("\r") + 1);
                                if (Constant.DEBUG) {
                                    System.out.println("RLY 18  = " + Variable.reply_18);
                                }
                                Variable.got_reply_18 = true;
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
