...
public class Client_14 {
    ...
    public static void send_command(String cmd) {
        try {
            if (Constant.DEBUG) {
                System.out.println("CMD 14 : " + cmd);
            }
            cmd = cmd + "\r\n";
            Variable.got_reply_14 = false;
            Variable.reply_14 = "";
            temp_rly = "";
    ...
    public static void start_server(String host, int port) {
        new Thread() {
            ...
            public void run() {
                try {
                    if (Constant.DEBUG) {
                        System.out.println("HOST 14 : " + host);
                        System.out.println("PORT 14 : " + port);
                    }
                    ...
                    while (((len > -1))) {
                        try {
                            ...
                            if (temp_rly.contains("\r")) {
                                Variable.reply_14 = temp_rly;
                                temp_rly = temp_rly.substring(temp_rly.indexOf("\r") + 1);
                                if (Constant.DEBUG) {
                                    System.out.println("RLY 14  = " + Variable.reply_14);
                                }
                                Variable.got_reply_14 = true;
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
