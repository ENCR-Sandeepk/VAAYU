...
public class Client_15 {
    ...
    public static void send_command(String cmd) {
        try {
            if (Constant.DEBUG) {
                System.out.println("CMD 15 : " + cmd);
            }
            cmd = cmd + "\r\n";
            Variable.got_reply_15 = false;
            Variable.reply_15 = "";
            temp_rly = "";
    ...
    public static void start_server(String host, int port) {
        new Thread() {
            ...
            public void run() {
                try {
                    if (Constant.DEBUG) {
                        System.out.println("HOST 15 : " + host);
                        System.out.println("PORT 15 : " + port);
                    }
                    ...
                    while (((len > -1))) {
                        try {
                            ...
                            if (temp_rly.contains("\r")) {
                                Variable.reply_15 = temp_rly;
                                temp_rly = temp_rly.substring(temp_rly.indexOf("\r") + 1);
                                if (Constant.DEBUG) {
                                    System.out.println("RLY 15  = " + Variable.reply_15);
                                }
                                Variable.got_reply_15 = true;
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
