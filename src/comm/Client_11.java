    ...
    public static void start_server(String host, int port) {
        new Thread() {
            ...
            public void run() {
                try {
                    if (Constant.DEBUG) {
                        System.out.println("HOST 11 : " + host);
                        System.out.println("PORT 11 : " + port);
                    }
                    ...
                    while (((len > -1))) {
                        try {
                            ...
                            if (temp_rly.contains("\r")) {
                                Variable.reply_11 = temp_rly;
                                temp_rly = temp_rly.substring(temp_rly.indexOf("\r") + 1);
                                if (Constant.DEBUG) {
                                    System.out.println("RLY 11  = " + Variable.reply_11);
                                }
                                Variable.got_reply_11 = true;
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
