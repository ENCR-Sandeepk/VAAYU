...
public class Client_4 {
    ...
    public static void send_command(String cmd) {
        try {
            if (Constant.DEBUG) {
                System.out.println("CMD 4 : " + cmd);
            }
            cmd = cmd + "\r\n";
            Variable.got_reply_4 = false;
            Variable.reply_4 = "";
            temp_rly = "";
    ...
    public static void start_server(String host, int port) {
        new Thread() {
            ...
            public void run() {
                try {
                    if (Constant.DEBUG) {
                        System.out.println("HOST 4 : " + host);
                        System.out.println("PORT 4 : " + port);
                    }
                    ...
                    while (((len > -1))) {
                        try {
                            ...
                            if (temp_rly.contains("\r")) {
                                Variable.reply_4 = temp_rly;
                                temp_rly = temp_rly.substring(temp_rly.indexOf("\r") + 1);
                                if (Constant.DEBUG) {
                                    System.out.println("RLY 4  = " + Variable.reply_4);
                                }
                                Variable.got_reply_4 = true;
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
