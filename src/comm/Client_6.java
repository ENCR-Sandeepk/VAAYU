...
public class Client_6 {
    ...
    public static void send_command(String cmd) {
        try {
            if (Constant.DEBUG) {
                System.out.println("CMD 6 : " + cmd);
            }
            cmd = cmd + "\r\n";
            Variable.got_reply_6 = false;
            Variable.reply_6 = "";
            temp_rly = "";
    ...
    public static void start_server(String host, int port) {
        new Thread() {
            ...
            public void run() {
                try {
                    if (Constant.DEBUG) {
                        System.out.println("HOST 6 : " + host);
                        System.out.println("PORT 6 : " + port);
                    }
                    ...
                    while (((len > -1))) {
                        try {
                            ...
                            if (temp_rly.contains("\r")) {
                                Variable.reply_6 = temp_rly;
                                temp_rly = temp_rly.substring(temp_rly.indexOf("\r") + 1);
                                if (Constant.DEBUG) {
                                    System.out.println("RLY 6  = " + Variable.reply_6);
                                }
                                Variable.got_reply_6 = true;
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
