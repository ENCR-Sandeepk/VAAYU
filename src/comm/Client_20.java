...
public class Client_20 {
    ...
    public static void send_command(String cmd) {
        try {
            if (Constant.DEBUG) {
                System.out.println("CMD 20 : " + cmd);
            }
            cmd = cmd + "\r\n";
            Variable.got_reply_20 = false;
            Variable.reply_20 = "";
            temp_rly = "";
    ...
    public static void start_server(String host, int port) {
        new Thread() {
            ...
            public void run() {
                try {
                    if (Constant.DEBUG) {
                        System.out.println("HOST 20 : " + host);
                        System.out.println("PORT 20 : " + port);
                    }
                    ...
                    while (((len > -1))) {
                        try {
                            ...
                            if (temp_rly.contains("\r")) {
                                Variable.reply_20 = temp_rly;
                                temp_rly = temp_rly.substring(temp_rly.indexOf("\r") + 1);
                                if (Constant.DEBUG) {
                                    System.out.println("RLY 20  = " + Variable.reply_20);
                                }
                                Variable.got_reply_20 = true;
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
