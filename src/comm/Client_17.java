...
public class Client_17 {
    ...
    public static void send_command(String cmd) {
        try {
            if (Constant.DEBUG) {
                System.out.println("CMD 17 : " + cmd);
            }
            cmd = cmd + "\r\n";
            Variable.got_reply_17 = false;
            Variable.reply_17 = "";
            temp_rly = "";
    ...
    public static void start_server(String host, int port) {
        new Thread() {
            ...
            public void run() {
                try {
                    if (Constant.DEBUG) {
                        System.out.println("HOST 17 : " + host);
                        System.out.println("PORT 17 : " + port);
                    }
                    ...
                    while (((len > -1))) {
                        try {
                            ...
                            if (temp_rly.contains("\r")) {
                                Variable.reply_17 = temp_rly;
                                temp_rly = temp_rly.substring(temp_rly.indexOf("\r") + 1);
                                if (Constant.DEBUG) {
                                    System.out.println("RLY 17  = " + Variable.reply_17);
                                }
                                Variable.got_reply_17 = true;
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
