...
public class Client_19 {
    ...
    public static void send_command(String cmd) {
        try {
            if (Constant.DEBUG) {
                System.out.println("CMD 19 : " + cmd);
            }
            cmd = cmd + "\r\n";
            Variable.got_reply_19 = false;
            Variable.reply_19 = "";
            temp_rly = "";
    ...
    public static void start_server(String host, int port) {
        new Thread() {
            ...
            public void run() {
                try {
                    if (Constant.DEBUG) {
                        System.out.println("HOST 19 : " + host);
                        System.out.println("PORT 19 : " + port);
                    }
                    ...
                    while (((len > -1))) {
                        try {
                            ...
                            if (temp_rly.contains("\r")) {
                                Variable.reply_19 = temp_rly;
                                temp_rly = temp_rly.substring(temp_rly.indexOf("\r") + 1);
                                if (Constant.DEBUG) {
                                    System.out.println("RLY 19  = " + Variable.reply_19);
                                }
                                Variable.got_reply_19 = true;
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
